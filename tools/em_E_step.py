...
def train(cfg, local_rank, distributed, logger):
    ...
    for iteration, (images, targets, _) in enumerate(train_data_loader):
        ...
        for t, logits in zip(targets, relation_logits):
            clip_data = t.get_field("clip_data")
            clip_data = modify_logits(clip_data, logits)
            img_path = clip_data['img_path']
            if img_path in dic:
                end = True
                break
            else:
                to_save.append(clip_data)
                dic[img_path] = None
        if end:
            break
    print(len(dic))
...
def modify_logits(clip_data, rel_logits):
    possible_rels = clip_data['possible_rels']
    ...
    clip_data['rel_logits'] = rst_rel_logits
    clip_data['possible_rels'] = rst_possible_rels
    return clip_data
