...
class AttnModule(nn.Module):
    def __init__(self, na_weight=1., feature_dim=4096, n_cls=21):
        super(AttnModule, self).__init__()
        self.n_cls = n_cls
        self.softmax = nn.Softmax(1)
        self.attn_fc = nn.Linear(feature_dim, n_cls)
        self.na_weight = na_weight
        # self.sft_temperature = sft_temperature

    def eval_forward(self, union_features, labels):
        device = union_features.device

        raw_attn_score = self.attn_fc(union_features)
        attn_score = torch.zeros((union_features.size(0), 21), dtype=union_features.dtype).to(device)
        attn_score[:] = 0.
        for i, lb in enumerate(labels):
            idxs = lb.topk(k=10, dim=0)[1]
            cur_attn_score = raw_attn_score[i, idxs]
            # modify self.softmax to nn.Softmax(0)
            # print(cur_attn_score)
            cur_attn_score = F.softmax(cur_attn_score).type_as(attn_score)
            attn_score[i, idxs] = cur_attn_score
        return attn_score

    def forward(self, union_features, labels):
        labels = cat(labels, dim=0)
        device = union_features.device
        # faster implementation
        # if not slow:
        nonzero_idxs = torch.nonzero(labels)
        raw_attn_score = self.attn_fc(union_features)
        to_fill_in = torch.zeros((union_features.size(0), self.n_cls), dtype=raw_attn_score.dtype).to(device)
        to_fill_in[:] = -10000
        to_fill_in[nonzero_idxs[:, 0], nonzero_idxs[:, 1]] = raw_attn_score[nonzero_idxs[:, 0], nonzero_idxs[:, 1]]
        to_fill_in = self.softmax(to_fill_in)
        attn_score = torch.zeros((union_features.size(0), self.n_cls), dtype=torch.float32).to(device)
        attn_score[:] = self.na_weight
        ...
        # else:
        #     raw_attn_score = self.attn_fc(union_features)
        #     attn_score = torch.zeros((union_features.size(0), 21), dtype=torch.float32).to(device)
        #     attn_score[:] = self.na_weight
        #     for i, lb in enumerate(labels):
        #         idxs = torch.nonzero(lb)
        #         cur_attn_score = raw_attn_score[i, idxs]
        #         cur_attn_score = F.softmax(cur_attn_score)
        #         attn_score[i, idxs] = cur_attn_score
        # print(attn_score[0])
        return attn_score


@registry.ROI_RELATION_PREDICTOR.register("UnionPredictor")
class UnionPredictor(nn.Module):
    def __init__(self, config, in_channels):
        super(UnionPredictor, self).__init__()
        self.num_obj_cls = config.MODEL.ROI_BOX_HEAD.NUM_CLASSES
        self.num_rel_cls = config.MODEL.ROI_RELATION_HEAD.NUM_CLASSES
        # self.use_bias = config.MODEL.ROI_RELATION_HEAD.PREDICT_USE_BIAS
        ...
        self.context_layer = UnionPair(config, self.num_obj_cls, self.num_rel_cls, in_channels)
    ...
    def forward(self, proposals, rel_pair_idxs, rel_labels, rel_binarys, roi_features, union_features, logger=None):
        ...
        obj_dists, rel_dists = self.context_layer(roi_features, proposals, union_features, logger)
...
@registry.ROI_RELATION_PREDICTOR.register("KBPredictor")
class KBPredictor(nn.Module):
    def __init__(self, config, in_channels):
        super(KBPredictor, self).__init__()
        self.attribute_on = config.MODEL.ATTRIBUTE_ON
        self.num_obj_cls = config.MODEL.ROI_BOX_HEAD.NUM_CLASSES
        self.num_att_cls = config.MODEL.ROI_ATTRIBUTE_HEAD.NUM_ATTRIBUTES
        self.num_rel_cls = config.MODEL.ROI_RELATION_HEAD.NUM_CLASSES
    ...
    def forward(self, proposals, rel_pair_idxs, rel_labels, rel_binarys, roi_features, union_features, logger=None):
        ...
        tail_reps = tail_rep.split(num_objs, dim=0)
        obj_preds = obj_preds.split(num_objs, dim=0)
        # print(proposals[0].fields(), proposals[0].get_field("labels"), obj_preds[0])
        obj_preds = [p.get_field("labels") for p in proposals]
        prod_reps = []
        pair_preds = []
        for pair_idx, head_rep, tail_rep, obj_pred in zip(rel_pair_idxs, head_reps, tail_reps, obj_preds):
            prod_reps.append(torch.cat((head_rep[pair_idx[:, 0]], tail_rep[pair_idx[:, 1]]), dim=-1))
        ...
        # rel_dists[:,[31]] = -20000.
        obj_dists = obj_dists.split(num_objs, dim=0)
        if proposals[0].has_field("predict_logits"):
            obj_dists = [p.get_field("predict_logits") for p in proposals]
        rel_dists = rel_dists.split(num_rels, dim=0)
...
class FreqPredictor(nn.Module):
    def __init__(self, config, in_channels):
        super(FreqPredictor, self).__init__()
        self.attribute_on = config.MODEL.ATTRIBUTE_ON
        self.num_obj_cls = config.MODEL.ROI_BOX_HEAD.NUM_CLASSES
        self.num_att_cls = config.MODEL.ROI_ATTRIBUTE_HEAD.NUM_ATTRIBUTES
        self.num_rel_cls = config.MODEL.ROI_RELATION_HEAD.NUM_CLASSES
