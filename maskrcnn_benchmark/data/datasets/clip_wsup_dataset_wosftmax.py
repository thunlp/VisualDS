...
class ClipDatasetWosftmax(torch.utils.data.Dataset):
    ...
    def __init__(self, split, img_dir, roidb_file, dict_file, image_file, transforms=None,
                 ...
                 custom_path='', distant_supervsion_file=None, clip_data_file=None):
        """
        ...
        self.clip_data = pickle.load(open(clip_data_file, "rb"))

        # check
        assert len(self.clip_data) == len(self.img_info), "{}, {}".format(len(self.clip_data), len(self.img_info))
        mask = [k is not None for k in self.clip_data]
        self.clip_data = [d for m, d in zip(mask, self.clip_data) if m]
        ...
        #for b in range(len(self.clip_data)):
        #    self.clip_data[b]['img_path'] = '/home1/private/zengzheni/dataset/'+self.clip_data[b]['img_path'][9:]
        for a, b in zip(self.clip_data, self.filenames):
            #print(a['img_path'], b)
            assert a['img_path'] == b, "{}, {}".format(a['img_path'], b)
    ...
    def __getitem__(self, index):
        ...
        target.add_field("clip_data", self.clip_data[index])
    ...
    def get_groundtruth(self, index, flip_img=False):
        ...
        for i, (logit, ds_rels) in enumerate(zip(rel_logits, possible_rels)):
            if logit is None:
                ...
                logit = (torch.from_numpy(logit).float().squeeze(0))
                #if np.abs(logit.sum()- 1.) < 1e-5:
                #    logit = (torch.from_numpy(logit).float().squeeze(0))
                #else:
