# pretrain
OUTPATH=$EXP/20/sgdet/semi/m_step1_pretrain
mkdir -p $OUTPATH
...
CUDA_VISIBLE_DEVICES=2,3 python -m torch.distributed.launch \
  ...
  SOLVER.SCHEDULE.MAX_DECAY_STEP 2   MODEL.ROI_HEADS.DETECTIONS_PER_IMG 64  TEST.INFERENCE "SOFTMAX"


# finetune
OUTPATH=$EXP/20/sgdet/semi/m_step1
mkdir -p $OUTPATH
cp $EXP/20/sgdet/semi/m_step1_pretrain/last_checkpoint  $OUTPATH/last_checkpoint   # finetune based  on pretrained model

CUDA_VISIBLE_DEVICES=2,3 python -m torch.distributed.launch --master_port 10083 --nproc_per_node=2 \
  ...
  SOLVER.BASE_LR 0.001 SOLVER.SCHEDULE.MAX_DECAY_STEP 2   MODEL.ROI_HEADS.DETECTIONS_PER_IMG 64
