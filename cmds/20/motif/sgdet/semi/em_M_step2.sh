# pretrain
OUTPATH=$EXP/20/sgdet/semi/m_step2_pretrain
mkdir -p $OUTPATH
cp $EXP/20/predcls/semi/e_step2/em_E.pk $OUTPATH/em_E.pk
...
CUDA_VISIBLE_DEVICES=4,5 python -m torch.distributed.launch \
  ...
  SOLVER.SCHEDULE.MAX_DECAY_STEP 1    MODEL.ROI_HEADS.DETECTIONS_PER_IMG 64  TEST.INFERENCE "SOFTMAX"


# finetune
OUTPATH=$EXP/20/sgdet/semi/m_step2
mkdir -p $OUTPATH
cp $EXP/20/sgdet/semi/m_step2_pretrain/last_checkpoint  $OUTPATH/last_checkpoint   # finetune based  on pretrained model

CUDA_VISIBLE_DEVICES=4,5 python -m torch.distributed.launch --master_port 10085 --nproc_per_node=2 \
  ...
  SOLVER.BASE_LR 0.001 SOLVER.SCHEDULE.MAX_DECAY_STEP 1    MODEL.ROI_HEADS.DETECTIONS_PER_IMG 64
