OUTPATH=$EXP/20/predcls/ds/m_step2_ex
mkdir -p $OUTPATH
cp $EXP/20/predcls/ds/m_step1_ex/last_checkpoint $OUTPATH/last_checkpoint # finetune based on m_step1 model
cp $EXP/20/predcls/ds/e_step2_ex/em_E.pk $OUTPATH # use e_step2 relabeled data
...
CUDA_VISIBLE_DEVICES=4,5 python -m torch.distributed.launch \
  ...
  WSUPERVISE.LOSS_TYPE  softwt WSUPERVISE.DATASET ClipDatasetWosftmax WSUPERVISE.CLIP_FILE  $OUTPATH/em_E.pk \
